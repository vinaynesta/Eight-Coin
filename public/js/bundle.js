...
var login = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(loginData) {
    var res;
        ...
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', 'You are successfully logged in');
...
var signUp = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(fdata) {
    var res;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', "You've successfully created account");
              window.setTimeout(function () {
                location.assign('/cpin');
              }, 1000);
            }
...
},{"axios":"../../node_modules/axios/index.js","./alert":"alert.js"}],"updateCoins.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.updateC = void 0;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alert.showAlert)('success', "You've successfully transfered coins");
              window.setTimeout(function () {
...
var _updateCoins = require("./updateCoins");
...
if (signupForm) {
  $("#signup-submit").click(function () {
    var a = JSON.stringify($('#sign-up-form').serialize());
    var res = a.replace(/=/g, '":"');
    res = res.replace(/&/g, '","');
    res = res.replace(/%40/g, '@');
    res = res.replace(/%20/g, ' ');
    var c = '{' + res + '}';
    var data = JSON.parse(c); //console.log(data)  
    //console.log(data.password,data.passwordConfirm);

    if (data.password === data.passwordConfirm) {
      (0, _signup.signUp)(data);
    } else {
      (0, _alert.showAlert)("error", "Your Passwords don't match");
    }
  });
}

var cpin = document.querySelector("#cpin");

if (cpin) {
  $("#cpin-submit").click(function () {
    var a = JSON.stringify($('#cpin-form').serialize());
    var res = a.replace(/=/g, '":"');
    res = res.replace(/&/g, '","');
    res = res.replace(/%40/g, '@');
    var c = '{' + res + '}';
    ...
    if (data.cpin === data.cpinConfirm) {
      (0, _updateSettings.updateData)(data, "cpin");
    } else {
      (0, _alert.showAlert)("error", "Your Passwords don't match");
    }
  });
...
var acc = document.querySelector(".user-details");
...
var prof = document.querySelector(".user-details");
...
if (loginForm) {
  $("#login-button").click(function () {
    var a = JSON.stringify($('#login-form').serialize());
    var res = a.replace(/=/g, '":"');
    res = res.replace(/&/g, '","');
    res = res.replace(/%40/g, '@');
    var c = '{' + res + '}';
    var loginData = JSON.parse(c);
    console.log(loginData, res, a);
    (0, _login.login)(loginData);
  });
}
...
var privatekey = document.querySelector("#private-key");

if (privatekey) {
  var pin1 = $("#user-profile").attr("data-val");
  $("#eye").click(function () {
    var pin2 = prompt("Please enter your Cpin");

    if (pin1 == pin2) {
      var a = $("#pk2").attr("data-key");
      $("#pk1").text(a);
      $("#eye").hide();
    } else {
      (0, _alert.showAlert)("error", "Wrong Cpin");
    }
  });
}

var editDetails = document.querySelector("#ep1");

if (editDetails) {
  $("#edit-details").click(function () {
    var a = JSON.stringify($('#ep11').serialize());
    var res = a.replace(/=/g, '":"');
    res = res.replace(/&/g, '","');
    res = res.replace(/%20/g, ' ');
    res = res.replace(/%40/g, '@');
    var c = '{' + res + '}';
    var editData = JSON.parse(c);
    var form = new FormData();
    form.append('photo', document.getElementById('photo').files[0]);

    for (var i in editData) {
      form.append("".concat(i), editData[i]);
    } //console.log(form)  
...
var cur = document.querySelector("#cx1");
...
if (cur) {
  ...
  $("#currencies").click(function () {
    var c = $('#currencies').find(":selected").val();
    var a = $("#amount").val();
    a = parseInt(a);
    value = currencyexchange[c] * a;
    console.log(value);
    $("#setCurrVal").text("".concat(value));
  });
  $("#buy-button").click(function () {
    if (value != 0) {
      var c = $("#getbal").attr("data-coin");
      c = parseFloat(c);
      c += value;
      var form = {
        coinBalance: c
      };
      (0, _updateSettings.updateData)(form, "coins");
    }
  });
  $("#ads1").click(function () {
    var c = $("#numberOfCoins").val();
    var bal = $("#bal").val();
    c = parseFloat(c);
    bal = parseFloat(bal);

    if (c > bal) {
      (0, _alert.showAlert)("error", "Insufficient balance");
    } else {
      var rpuba = $("#receiverPublicAddress").val();
      var spuba = $("#ads1").attr("data-publickey");
      var fdata = {
        name: "Transfer Coins",
        receiverPublicAddress: rpuba,
        senderPublicAddress: spuba,
        sentCoins: c
      };
      (0, _updateCoins.updateC)(fdata);
    }
  });
  $("#currencies1").click(function () {
    var c = $('#currencies1').find(":selected").val();
    var a = $("#amount1").val();
    a = parseInt(a);
    value = currencyexchange[c] * a; //console.log(value);

    $("#setCurrVal1").text("".concat(value));
  });
  $("#transfer-cash").click(function () {
    var c = $("#setCurrVal1").text();
    c = parseFloat(c);
    var bal = $("#bal").text();
    bal = parseFloat(bal);
    ...
    if (c > bal) {
      ...
      var rpuba = $("#receiverPublicAddress1").val();
      var spuba = $("#ads1").attr("data-publickey");
      var name = "Transfer Cash";
      var fdata = {
        receiverPublicAddress: rpuba,
        senderPublicAddress: spuba,
        sentCoins: c,
...
if (searchParams.has('coins')) {
  var param = searchParams.get('coins');
...
if (searchParams.has('name')) {
  ...
  var _param = searchParams.get('name');

  _param = _param.replace(/%20/g, " ");
  $("#orderName").text(_param);
}
...
$("#pay").click(function () {
  ...
  c = parseFloat(c);
  var rpuba = $("#receiverPublicAddress").val();
  var spuba = $("#puk").attr("data-publickey");
...
var _loop = function _loop(i) {
  $("#b".concat(i)).click(function () {
    var name = $("#ab".concat(i)).data("name");
    var coins = $("#ab".concat(i)).data("coins");
    coins = parseInt(coins);
    location.assign("/paymerchant?coins=".concat(coins, "&name=").concat(name));
  });
};
